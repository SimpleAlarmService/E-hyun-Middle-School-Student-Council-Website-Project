 ...
 * (ALL_TI_YMD 단일 날짜 쿼리 × 5 → Promise.all)
 */
...
export function useTimeTable(
  ...
  useEffect(() => {
    ...
    Promise.all(weekDates.map((d, i) => fetchDay(d, i)))
      .then(results => {
        const g: TimetableGrid = {};
        results.forEach(({ dayIdx, periods }) => {
          g[dayIdx] = periods;
        });
        setGrid(g);
      })
  ...
  return { grid, weekDates, maxPeriod, isLoading, error };
}
